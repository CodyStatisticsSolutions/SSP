...
moderation.anova <- function(iv, mv, dv, data){

	formula <- as.formula(paste(dv, "~ as.factor(",iv,") + as.factor(",mv,") + as.factor(",iv,")*as.factor(",mv,")"));
...
#moderation.anova("DifferentGroup3", "Group3", "NotNormalDependent", data=data)
